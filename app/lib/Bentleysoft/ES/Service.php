...
//                          'subject.ldcode'=>$extraTerms,
    ...
    if ($showViewable && $showUnviewable) {
      $filters[1]['or'][] = array("term"=>array("viewable"=>'1'));
      $filters[1]['or'][] = array("term"=>array("viewable"=>'0'));
    ...
    if (!empty($areas)) {
      $filters[]['and'][] = array('term'=>array("subject.id"=>$extraTerms ));
    }
...
///    $filters[]['and'][] = array('term'=>array("subject.id"=>$extraTerms ));
   ...
   * @return mixed
   */
  public static function test($from = 0, $size = 20, $pattern = '*', $sector='FE')
  {
    $searchParams['index'] = 'ciim';
    ...
    $must = array();
    $must[] = array("query_string"=>array("query"=>$pattern));
    $must[] = array( 'terms'=>array('sector'=>array('FE')) );
  ...
  public static function getLdCodesForSubjects($subjectIds = array()) {
    ...
    foreach ($subjectIds as $i=>$sid) {
      ...
      foreach ($s2ls as $s2l) {
        ...
        $ret[] = strtoupper($learnDirect->ldcs_code);
      }
    }
    return $ret;
  }
